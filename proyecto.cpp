...
int main(){
    int opcion,leche,tamanio,cantidad,e;
    char sino;
    float temperatura;
    bool continua = true;
    ...
    while (continua == true){    
        ...
        if(opcion == 1){
            ...
            cin>>tamanio;
            cout<<"Quiere espuma en su capuccino? 1. Si 2. No  ";
            cin>>e;
            cout<<"\n";
            cantidad = convierte_tamanios(tamanio);
            obj.agrega_capuccino(leche,temperatura,cantidad, e);
        ...
        cout<<"Desea cotinuar ordenando? y/n ";
        cin>>sino;
        if((sino == 'y') or (sino == 'Y')){
            continua = true;
        }else if((sino == 'n') or (sino == 'N')){
            continua = false;
            obj.pago_total();
        }else{
            break;
