...
class Orden{
    ...
    public:
        Orden();
        void agrega_capuccino(int leche, float temperatura, int cantidad,int e);
...
void Orden::agrega_capuccino(int leche,float temperatura,int cantidad,int espuma){
    icapuccino += 1;
    Capuccino aux(leche,temperatura,cantidad,espuma);
...
void Orden::pago_total(){
    ...
    for(int i = 0; i <= icapuccino; i++){
        total = total + lista_capuccino[i].calcula_precio();
    }
    for(int i = 0; i <= iamericano; i++){
        total = total + lista_americano[i].calcula_precio();
    }
    for(int i = 0; i<= ilatte; i++){
        total = total +  lista_latte[i].calcula_precio();
    }
